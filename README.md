...
The concrete sign gap is now closed: Lean proves
`defect(6,g)<3^R_6(g)` and that the defect is positive and odd.  Hence a
signed reset chain with nonnegative initial value must start strictly positive
and stays strictly positive forever.  For this successor program, eventual
zero carry is therefore **equivalent** to existence of the unique positive
natural full-return ray; positivity is not a second open gate.

A literature audit puts the unique 2-adic source into an exact six-value
Tschakaloff linear form.  The apparent seventh value is removed by
`f_q(qx)=1+x f_q(x)`, with `q=3^102/2^138`.  The 1989
Väänänen--Wallisser six-value size bound fails (`gamma=0.146389...` versus
`Gamma(6,0)=0.079867...`).  Rochev's 2011 theorem is the closest broader
near-hit: it would handle arbitrary finite families without that rank gate,
but assumes one expanding place, whereas this `q` expands both really and
2-adically.  The same inequality `3^17>2^23` which makes the return pay its
real precision bill creates the second expanding place.  The high-leverage
arithmetic target is consequently a two-expanding-place, fixed-linear-form
Hermite--Padé argument, not more finite carry scanning.  See
[`long-return-successor-literature.md`](docs/notes/long-return-successor-literature.md).
...
The first distinguished nonlinear return has now been closed at full
positive-integer strength.  For the legal base-squaring quine
`g -> 2g`, a reduced rational payload `N/Q` forces both
`Q(z)|Q(kappa z^2)` and `Q(kappa z^2)|z^2 Q(z)`.  Lean derives
`deg Q<=2` and classifies every survivor: a constant, `z`, `z^2`, or the
simple fixed pole `1-kappa z`.  The first three reduce to explicit unmatched
coefficients; cancellation at the fixed pole would require the false identity
`2*A*kappa^2=D`.  Thus **no rational function** supplies the special payload.
More decisively, the exact cleared integer balance factors as
`3^(2Q)(A F-b0)=2^(77+P)(positive)`, forcing
`2^(131+23g)<A F(g)`.  Positivity of the same return gives the incompatible
upper law `F(2g)<2^(46+8g)F(g)`.  Along `g,2g,4g,...`, the lower precision
grows with slope `23` while the complete Archimedean budget grows with slope
`8`; Lean obtains a contradiction at the explicit depth `N=F(0)+4`.
...
payload natural under a second independent `g->3g` dilation.  Those theorems
alone do not touch a log-singular homogeneous section on one dyadic opcode
ray, but the elementary precision-versus-height argument above does:
integrality is exactly the extra boundedness that destroys it.  The 5x+1
control still warns that parity or 2-adic dynamics alone cannot settle a
general construction.  A survivor must evade this new two-place slope
obstruction—its autonomous return must create at least as much real height as
the dyadic precision it forces.  No such architecture or counterexample is
known or claimed.
...
`g -> 2g` doubling return, **2-adic value rigidity** first annihilates the
QM184c non-holomorphic freedom on every integer-payload ray (QM185).  A new
kernel proof then removes the former irrationality seam altogether: exact
divisibility forces `23g` dyadic precision, while positivity permits only an
accumulated `8g` Archimedean exponent, so no positive integer ray exists
([notes](docs/notes/doubling-payload-rigidity.md), QM187).  (3) In the full
...
Dima Sinapova separates several exact reflection principles at successors of
singular cardinals.  Countable simultaneous reflection forces SCH at a
countable-cofinality singular, while relative-consistency results obtain
failure of SCH with every fixed finite simultaneous reflection; her new
uncountable-cofinality theorem proves ordinary reflection, while the advertised
generalization and countable simultaneous version are not yet checked.  The
Collatz lesson is the quantifier order: every fixed finite family of checks may
share a depth-dependent bound `B_r` although no single finite `B` handles all
checks.  Increasing audit depth cannot replace one cutoff-independent repair
map or potential.

Andrei Rapinchuk proves finiteness of the genus of a division algebra over a
finitely generated field by showing that equal maximal-subfield data forces
the same ramification support and leaves only a finite unramified Brauer
kernel.  Related algebraic-group results use good reduction, while several
general triviality statements remain conjectural.  A Collatz local fingerprint
can similarly narrow a controller only to a certified finite ambiguity class.
Every residual representative must still be replayed; equality of tested local
data is singleton identification only after the kernel is proved trivial.

Florian Richter proves that every positive-density subset of the naturals
contains `B_1+...+B_k` with all `B_i` infinite, using correspondence, Host--Kra
cubes, and a continuous disintegration that pins the first coordinate to a
specified transitive point.  The conclusion is automatic for every finite
Collatz cylinder: if `C={r mod q}`, take `B_1={r+qm:m>=1}` and
`B_i={qm:m>=1}`.  The all-odd cylinders have this richness at every depth yet
nest only to `-1`, so higher-order recurrence does not furnish cross-depth
ordinary coherence.

Lap Chi Lau solves the Paulsen problem up to a polynomial dimension factor by
replacing zigzagging discrete operator scaling with a continuous gradient flow
and a capacity invariant.  The exact arithmetic transfer is a gap certificate.
For a fixed word let `R(n)=(2^A-3^k)n-B`; a proof that the integer `R(n)` has
absolute value below one forces `R(n)=0`.  Continuous convergence alone is
insufficient: for word `123`, `R(n)=37n-23` vanishes at the coherent rational
cycle root `23/37` and at no integer.

Philip Engel turns an assumed algebraic curve on a matroidal degeneration into
a universal colored graph and then a finite-field linear/quadratic obstruction,
proving integral-Hodge and stable-irrationality consequences.  This is a useful
one-way architecture test if every ordinary Collatz realization is first
proved to induce the graph.  Local solvability cannot reverse the implication:
`(x^2-13)(x^2-17)(x^2-221)` has a root over every `Z_p` but no integer root.
A family of satisfiable modular carry systems therefore is not an ordinary
seed.

Ofer Gabber presents the finiteness proof for étale cohomology over excellent
schemes by controlled alterations, log-regular models, purity, and descent.
The proof move suggests auxiliary finite carry charts only with an exact
projection/descent theorem.  Its splitting step is explicitly prime to the
coefficient characteristic: for a two-sheeted cover with `F_2` coefficients,
pullback followed by trace is multiplication by two and hence zero.  Collatz's
critical branching is dyadic, so tame prime-to-two alteration cannot be
assumed to regularize it, and adic approximation alone supplies no Archimedean
root bound.

Formalizer commit `8927732` closes the literal writer--decoder seam and rules
out a vast finite-chart repair.  Every literal coarse chart occupies at most
50 classes modulo `2^54`; any portfolio of at most `360,287,970,189` charts
leaves a common dyadic class, and CRT places arbitrarily large ordinary
parameters from every prescribed ternary cylinder in that common hole.
Commit `d2375b2` separately removes the finite-grammar hypothesis from the
Kraft decay: every finite family of schedules with exactly `n` first-passage
blocks has total dyadic mass at most `(17/20)^n`.  Commit `fc295a0` gives the
literal cylinder form: at dyadic depth `L`, the proportion of residue classes
beginning `n` complete recharges is at most `(17/20)^n`.  The modules rebuild.
These are an exact finite-portfolio no-go and full-language density theorems,
not an exclusion of an unbounded recursive selector or one measure-zero
infinite schedule with an ordinary root.  `counterexample:null`.
...
Chris Eur proves that the exponential generating polynomial counting distinct
rainbow panels assembled from fixed colored committees is Lorentzian; its line
restrictions have no internal zeros and are log-concave.  This gives a compact
architecture test.  If a proposed independent colored-slot macro model has
endpoint allocations `(2,0)` and `(0,2)` but no mixed `(1,1)` allocation, it
cannot have the claimed factorization.  Collatz blocks are ordered and
carry-coupled, so the theorem rejects a false independence model rather than
creating an exchange; log-concavity also gives neither drift nor a nested
ordinary root.

Maxwell Aires reports new asymptotic balance theorems for random linear
extensions of wide or extension-rich posets, while the one-third/two-thirds
and Kahn--Saks conjectures remain open.  Exact dynamic programming over ideals
can use the proved `3/11` close-rank theorem to select a comparison that removes
a constant fraction of finite proof schedules, a principled CEGIS branching
rule.  But two macros can occur in either order in a dependency poset while
only one composition replays.  An antichain is perfectly balanced although a
macro graph on its vertices may have no path longer than one.

Maria Chudnovsky proves that excluding explicit induced obstruction families
forces bounded pathwidth, using minimal branch-set shapes, Ramsey selection,
and a recursive unbreakable-banana argument.  Applied to an exact finite
prefix-compatibility graph, the theorem could return a small induced witness
against a bounded-pathwidth selector architecture.  It cannot prove escape:
the nested all-odd cylinders form a compatibility path of pathwidth one, have
literal witnesses `2^(H+1)-1`, and still converge only to `-1` with unbounded
representatives.

Subhroshekhar Ghosh proves outside-number rigidity for Ginibre points and
outside-number-plus-sum rigidity for Gaussian analytic-function zeros, with
conditional tolerance on the remaining constraint surface.  A Collatz tail
statistic must reconstruct the ordered affine carry, not merely symmetric
moments.  Words `123` and `132` have identical valuation multisets and every
symmetric moment but carries `23` and `31`, cylinders `11` and `115 mod 128`,
and distinct rational cycles.  The talk's explicit warning that rigidity need
not survive weak limits matches the ordinary-root gate exactly.

Dan Cristofaro-Gardiner proves a two-term ECH-capacity asymptotic for generalized
convex toric domains and derives an affine-perimeter obstruction to full
fillings; the final systolic tiling theorem is described as work in progress.
A density-plus-boundary Collatz invariant is location-blind.  In the dyadic
cycle, singleton cylinders `1 mod 2^m` and `-1 mod 2^m` have the same density
and edge perimeter two.  The first family stabilizes to ordinary seed `1`; the
second has representatives `2^m-1` and limit `-1`.  Any useful arithmetic
capacity must include a proper height/location term.

Thierry de la Rue proves that generalized Veech orthogonality is controlled by
the maximal factor in a characteristic class, using joinings of visible
empirical measures; the Möbius instance remains conjectural.  Visible measures
can erase infinitely many sparse failures.  The all-zero binary sequence and
the sequence equal to one exactly at powers of two have the same sole
Furstenberg measure, since every block containing a one has zero density, yet
the second has infinitely many bad events.  Average factorization therefore
cannot certify perpetual Collatz legality or exclude sparse returns.

Formalizer commit `a0fba3a` transports the sharp Kraft theorem to literal
shortcut semantics.  At every depth `L`, at least `3/20` of actual residue
classes modulo `2^L` have an exact `L`-step execution with no outward prefix,
and every such class has arbitrarily large positive ordinary lifts with the
same execution.  The module rebuild passes.  This is a genuine finite
ordinary-density theorem, but the many residue sets have not been given
compatible bonding maps selecting one fixed natural number at all depths.

These talks and the new formal theorem improve finite architecture rejection
and show that noncrossing is arithmetically abundant at every separate depth.
They do not supply nested stabilization, perpetual recharge, coercive height
growth, or a positive ordinary infinite orbit.  `counterexample:null`.
...
Formalizer commits `cb8e92a`, `e789979`, and `ed3576c` now kernel-check a
universal first-passage scarcity theorem.  The sharpened exact comparison
`3^3 < 2^5` gives `3*false < 2*true` for every outward word; a rational
weighted Kraft proof then bounds the complete prefix-free first-passage mass
by `17/20`.  At every finite depth at least `3/20` of all Boolean words have
no outward prefix.
...
Guy Blanc proves a tight direct-product/hardcore phenomenon for compositions
with independent random balanced inner functions: weak one-block advantage
cannot be boosted to high accuracy without inspecting a growing number of
blocks.  This conditionally supports adding memory/features when exact CEGIS
failures remain ambiguous.  It does not apply directly because every Collatz
block is generated by the same deterministic arithmetic rule with strong
carry correlations; the talk explicitly notes that shared inner functions
break the lifting theorem.  A Collatz use first needs an exact independence or
correlation-decay theorem and a coupling from majority score to charge growth.

Jennifer Listgarten makes distributional search precise.  If a finite search
family contains all point masses, maximizing exact expected utility over
distributions equals maximizing over candidates, and a positive exact Boolean
expectation contains an extractable finite witness.  A learned oracle has no
such guarantee: for exact scores `0,-1` and surrogate scores `0,M`, it selects
the worse point.  Adaptive EM, density trust regions, and diversity are useful
proposal mechanisms only; every supported Collatz candidate still needs exact
replay and a one-sided theorem-backed score bound.

Pranav Chakravarthy constructs an integer-valued monotone spectral capacity
whose integer part survives the relevant conjugations and obstructs squeezing.
Total Collatz valuation is an integer monotone cylinder capacity, but it does
not see integrality.  All six permutations of `(1,2,3)` have total `6`; their
exact positive rational fixed roots are
`23,31,29,53,49,65` divided by `37`.  They form two literal rational
three-cycles and are coherent in `Z_2`, yet none is an ordinary integer.

Stefan Patrikis distinguishes quotient-valued compatible systems from full
lifts through a central cover, requiring canonical lifts and a uniqueness
argument.  Projected Collatz valuation data likewise has inequivalent lifts.
Words `123` and `132` have the same total and multiset, but affine constants
`23` and `31`, prefix cylinders `11` and `115` modulo `128`, and different
rational roots.  The hidden ordered carry is a genuine lifting variable.

Moritz Kerz's proper-base-change proof is a useful checklist: move every cycle
to good position, lift smooth special-fiber points surjectively, prove the lift
independent of choices, and show every defining relation descends.  Finite
Collatz residue cylinders satisfy only the easy surjective-lift step—each has
positive representatives.  The nested `-1 mod 2^N` example shows that no
natural bounded inverse is thereby supplied.  Lift independence, transition
relations, and ordinary-root descent must all be proved.

Zuo Lin proves an effective dichotomy between polynomial equidistribution and
polynomial proximity to a small periodic orbit.  This suggests an
exception-first finite search: explain every statistically anomalous long
survivor by exact shadowing of a classified 2-adic periodic orbit before
treating it as novel.  The all-odd representatives exactly shadow the fixed
point `-1` for arbitrary finite depth, so this alternative explains survival
without producing a positive root.  An aperiodic ordinary escape need not fit
the homogeneous theorem and remains untouched.

Formalizer commit `e97bd6d` answers the resource question exactly for the
canonical model: whenever the `n`th canonical recharge iterate from charge
`H` is defined with endpoint `K`, Lean proves `H+n <= K`.  It also proves no
positive canonical period, gives the explicit failure depth `B+1` for any
claimed charge bound `B`, and shows infinite first-passage execution has
unbounded canonical charges.  Thus the counters really must grow; the theorem
does not construct a forever-defined orbit.  `counterexample:null`.
...
Artem Chernikov's combinatorial group-configuration proposition gives a
compact exact architecture test.  For an `r`-ary relation with `r >= 4`,
unique completion in every coordinate and a permutation-invariant
rectangularity law are equivalent to expressing the relation, after
coordinate bijections, as a zero-sum equation in one abelian group.  On a
complete bounded recharge table these hypotheses can be exhaustively tested;
the first missing/double completion or failed rectangle is a finite rejection
witness, and a passing table yields a Cayley table that can be independently
replayed.  There is also a decisive limit: any homomorphism from the recovered
finite group to the additive integers is zero.  Positive recharge drift must
therefore be a separate cocycle or unbounded extension, not the finite group
coordinate itself.  Cross-precision group maps and ordinary-root
stabilization remain additional obligations.

Donggyu Kim supplies a particularly economical carry-coherence validator,
conditional on a genuine antisymmetric-matroid encoding.  The first homology
of the transversal base graph is generated by cycles of total exchange weight
at most eight.  Labeling each exchange edge by an exact abelian carry
correction therefore reduces all-cycle zero holonomy to a finite small-cycle
test.  Passing integrates the carry to a vertex potential; on a finite graph
that potential is bounded and cannot itself be the escape counter.  A
successful construction needs zero root/carry holonomy together with a
separately replayed positive counter cycle or unbounded coordinate, plus
compatible encodings at every precision.

Nina Kamčev gives an exact finite no-go for one possible multiplicity premise.
For a homogeneous finite-field system of even girth, if every minimum-support
row-space equation has a nonmatchable coefficient multiset, the system is
uncommon: some two-coloring has fewer monochromatic solutions than the random
benchmark.  Thus a genuinely homogeneous `F_3` encoding of bounded recharge
phases can be audited by enumerating its row space and all zero-sum pairings of
critical coefficients.  The theorem is silent for affine, directed, carry,
or inequality data lost by the encoding, and uncommonness is only an
aggregate density deficit, never a missing literal path.

Gil Kalai's clean exact transfer is the Sauer--Shelah trace certificate.  If
a family `F` of subsets of an `n`-set has size strictly greater than
`sum_{i=0}^m binom(n,i)`, some `(m+1)`-coordinate trace realizes every binary
pattern.  The strict inequality matters: all subsets of size at most `m`
attain equality without shattering an `(m+1)`-set.  Applied to an exactly
replayed finite macro language, this either certifies a complete feature
trace or bounds the number of signatures.  The patterns can be realized by
different macros and different ordinary seeds, so shattering is not a
selector, a growing counter, or cross-depth coherence.

Keith Frankston's fractional-threshold theorem suggests a principled CEGIS
refinement statistic.  A finite multihypergraph whose conditional edge counts
obey the exact `rho`-spread inequalities is hit by a random set at the stated
`rho log |X|` scale, and linear-programming duality identifies the optimal
spread measure with the fractional expectation threshold.  For a bounded
failure-witness language, exact rational degree ratios and the primal/dual LP
can distinguish failures concentrated on a small partial feature--a natural
architectural refinement--from failures dispersed across the current feature
grammar.  This is a population diagnostic: residualization does not choose a
nested ordinary seed, and the source theorem assumes a monotone finite set
family.

Julian Sahasrabudhe's simultaneous-book proof gives a related but more
geometric outer-loop feature.  When a greedy reservoir step loses too much
compatibility density, centered neighbourhood-incidence vectors turn that
loss into negative inner products; the proof then obtains either a usable
book step or a clustered subreservoir with a density increment.  For a finite
recharge table we can compute those rational incidence profiles exactly and
require every proposed step or refinement to carry a replayable certificate.
The transcript is a proof sketch and its displayed quantitative lemma is not
clear enough to import as a Collatz theorem, so an exact bounded instance must
be checked exhaustively.  Density increments can still follow different
seeds at every depth.

Formalizer commit `dd44b60` now proves a basic fact that sharply limits all of
these population analogies: literal recharge targets from one ordinary charge
form a chain, not a branching tree.  Any two common-source recharge words are
prefix-comparable, and the nonempty suffix between ordered targets is itself
a literal recharge macro.  A symbolic graph may retain transitive skips, but
...
Formalizer commit `af4c46f` has also discharged the finite-height premise
needed by this lane.  For every literal outward first-passage writer--decoder
cell at fixed positive source charge `H`, Lean proves

```text
2^(4*3^counter+counter+4+drain) < 9*H
```

and supplies an executable finite set `heightAdmissibleSymbols H` containing
every legal `(counter,drain)`.  It also kernel-checks the exact output
envelope used in the research note.  Thus a bounded CEGIS round can enumerate
the complete local alphabet without guessed cutoffs.  This does not bound the
...
Habegger's weighted core-entropy transfer has meanwhile become a
kernel-checked architecture filter in formalizer commit `4a1695e`.  For a
finite graph of Boolean first-passage words, an exact rational positive vector
with `Mv<=r*v` and `r<1` forces a state with Kraft mass below one; Lean then
constructs a literal finite Boolean word uncovered by every outgoing prefix at
that state.  This is the desired finite-state `rho(M)<1` hole theorem without
floating spectral numerics.  It does not cover an infinite-state mixed
recursive selector, identify the symbolic hole with an ordinary
writer--decoder cylinder, or rule out one exceptional atomic ray.

Raymond's resource-cone transfer is now kernel-checked as well in formalizer
commit `a7dfb30`.  A finite first-passage grammar may carry separate exact
rational resource scores.  If rational vertex potentials and nonnegative
resource multipliers make every edge's reduced score strictly negative, while
any admissible nonzero circulation would have nonnegative aggregate budgets,
Lean rules out that circulation by exact telescoping.  An LP may propose the
certificate, but none of its numerics are trusted.  The semantic premise and
...
Formalizer commit `f0576d6` adds the ordered version: exact vertex-potential
telescoping rules out every nonempty closed macro walk whose selected total
resource score is nonnegative under the same strict dual.  This is a direct
rejection certificate for a returned graph-state candidate, not an inference
from finite survival.
...
exact payload coordinates of a whole writer--decoder cell:

```text
H(p,b,Q)=(2^D(p,b)Q-B(p))/9
   -> 3^(p+o(p)+b)Q.                              (KC-FP16)
```

This exposes compact, exact architecture failures.  The unrestricted triple
predicate fails at `(p,b,Q)=(2,0,7)`, whose ordinary source is
`28022397681347377`; its output is `3^33*7` and has next-writer valuation two.
Adding the exact next-writer condition still fails at the canonical CRT
payload `Q=187`: the writer valuation is six, but the decoder sees only seven
of the required 55 bits.  Fixed residues `Q mod 2^k 3^ell` cannot repair an
unbounded target valuation: a residue-preserving perturbation makes the
target valuation exactly `k`.  The artifact checks `7<=k<=16`.

The executable audit also caught and corrected a tempting false simplification.
The mandatory ternary quotient is
...
not a two-node quotient excluding `v3(Q)=1`.  Finally, 81 exact ordinary
one-edge cylinders cover every prescribed pair in `2<=p,p'<=4`,
`0<=b,b'<=2`.  Local transition existence is therefore cheap; coherent
unbounded closure is the obstruction.  The next grammar must carry runtime
dyadic precision and a genuinely aperiodic or unbounded counter update.
The new artifact records `universal_invariant:null` and `counterexample:null`.
...
selector criterion rather than only a sequence of failed predictors.  With

```text
r_n=B_n/3^A_n,  x_n=2^D_n/3^A_n,
```

the chart recurrence is `r_(n+1)=r_n+x_n*Bg_n/9`.  One fixed natural root
exponent `C` executes every prescribed cell exactly iff
`r_infinity=-3^C` in `Q_2`.  For bounded symbols the same positive increments
converge over the reals.  There is no sign contradiction because rational
sequences can have different real and 2-adic limits.  There is, however, a
clean obstruction: any finite rational coboundary would telescope to the
same rational tail value at both places.  Therefore every eventually
periodic schedule, and more generally every finite-rational-coboundary tail,
is impossible.  The live bounded case is a genuinely aperiodic “two-place
chameleon” with factor complexity at least `k+1`.

An elementary size estimate supplies a second theorem-shaped filter.  Every
legal cell with writer counter `c` and decoder drain `b` satisfies

```text
9H > 2^(4*3^c+c+4+b).
```

So large counters require doubly exponential charge and should not be probed
by an undirected sweep.  Artifact schema v3 regression-checks the bounded
ingredients; the universal derivations are research-side pending QM167.
The record remains `universal_invariant:null`, `counterexample:null`.

### 2026-07-23 20:09 EDT

There is still no counterexample.  Exact replay of the first writer--decoder
beam supplied the next selector architecture rather than a reason to scale:
derive the canonical writer counter from `v2(9H+7)-4`, retain prefixes which
actually re-enter `010111`, and only then minimize the resonant-decoder
valuation shortfall.

The reconstructible semantic lane uses the same nine prefix symbols, depth
eight, beam 64, and derived frontier counters through six.  It checks 3,546
frontiers, exhaustive through depth two.  Writer re-entry is not the main
barrier: from depth two onward the beam repeatedly finds canonical prefixes
with `c>=2`.  None reaches the decoder threshold.  For `c=2` the required
valuation is 55; the best displayed value is 14, a 41-bit shortfall at depth
five.  The per-depth minimum shortfalls are

```text
null,145,45,47,41,46,46,46,
```

with zero valid zero-carry cells.  This rejects the semantic scoring
refinement at its stated bounds and records the exact first failure of each
architecture.  It is not evidence that the missing 41 bits can never occur.
Further first-crossing work now requires an arithmetic law which supplies the
decoder congruence at a fixed canonical exponent, not another beam increase.
The artifact still has `universal_invariant:null` and `counterexample:null`.
...
coefficient-matched recharge edge.  `OutwardChartRankNoGo.lean` now
kernel-checks the exact scoped rank law and rules out every finite nonempty
total graph of fixed chart instances by a minimum-rank argument.
...
- **KL-calibrated recharge/discharge rays.**  Treat the positive class-8
  ...
  In the newer writer--decoder invariant lane, two consecutive prescribed
  payload edges reduce exactly to `n=n0+2^D''*m`,
  `u=u0+3^(g+2)*m`: the next symbol consumes its full dyadic address depth
  and transports the remaining parameter in the other base.  This proves
  that finite unions of ternary parameter cylinders with a fixed finite
  target alphabet cannot be invariant.  More strongly, all counters
  `p>=51` share one writer class modulo `2^54`, while `2<=p<=50` contribute at
  most 49 further classes.  Thus even the full unbounded target alphabet has
  a dyadic-open complement meeting every ternary cylinder by CRT.  The live
  recursion must use the exact dyadic binder at every node.
...
| Complete finite-state first-passage router and full finite `n`-macro schedule families | Closed unconditionally at the density level.  Commits `cb8e92a`--`ed3576c` prove from exact `3^3<2^5` and a weighted Kraft tilt that every prefix-free outward family has fair dyadic mass at most `17/20`; the complete countable first-passage code obeys the same bound.  Commit `a0fba3a` proves that at every depth at least `3/20` of actual residue classes modulo `2^L` have literal no-outward-prefix executions and arbitrarily large positive ordinary lifts.  Commits `c5f8b92`/`d8fa08e` calibrate coherent nonordinary and trivial-cycle survivor rays.  Commit `29ac135` gives finite-grammar length-`n` mass at most `(17/20)^n`; commit `d2375b2` removes the grammar and proves the same bound for every finite family of uniquely parsed `n`-block schedules, with fixed-length integer bound `20^n*card<=17^n*2^L`.  Commit `fc295a0` unions their extension cylinders: at depth `L`, residue classes beginning `n` complete recharges have proportion at most `(17/20)^n`.  This is exponential rarity, not exclusion of one zero-mass infinite path or proof of its ordinary root. | [`OutwardStrictKraftGap.lean`](KontoroC/KontoroC/OutwardStrictKraftGap.lean), [`OutwardIteratedKraftGap.lean`](KontoroC/KontoroC/OutwardIteratedKraftGap.lean), [`OutwardSurvivorResidues.lean`](KontoroC/KontoroC/OutwardSurvivorResidues.lean), [`ias-talk-transfer-audit.md`](docs/notes/ias-talk-transfer-audit.md#lead-b-weighted-core-entropy-before-recursive-search) |
...
| Six-phase successor alphabet as an autonomous writer | Closed.  Equal source width and output stride do not create control freedom: every split `k+n+1=6` realizes the same deterministic `ReturnBalance(6,g,F,Fnext)`, so two enabled phases from one source have the same next source.  The phase-erased successor schedule is one fixed signed dyadic-reset program with unbounded cumulative precision.  At most one ordinary source can realize it.  Lean now proves `defect(6,g)<3^R_6(g)` and oddness, so eventual zero carry is equivalent to existence of the unique positive natural full-return ray, not merely a weak signed chain.  Its exact source is a six-value 2-adic Tschakaloff form; current published sufficient theorems miss because of rank or because `q=3^102/2^138` expands at both the real and 2-adic places. | [`LongReturnSuccessorZeroCarry.lean`](KontoroC/KontoroC/LongReturnSuccessorZeroCarry.lean), [`long-return-successor-literature.md`](docs/notes/long-return-successor-literature.md) |
...
- **A. Chernikov, [“Recognizing Groups in Erdős Geometry and Model
  Theory”](https://www.ias.edu/video/recognizing-groups-erdos-geometry-and-model-theory),
  IAS talk (28 Mar. 2023), corpus key
  `1a650621055c8ddba3528228228aedde`, S3/S5/S7 at 2,167--2,997 seconds** —
  unique completion and rectangularity recognize an abelian zero-sum
  relation for arity at least four.  This gives a complete finite selector
  architecture test, while a finite additive factor cannot carry nonzero
  homomorphic integer drift.
- **D. Kim, [“Baker-Bowler Theory for Lagrangian
  Grassmannians”](https://www.ias.edu/video/baker-bowler-theory-lagrangian-grassmannians),
  IAS talk (24 Oct. 2024), corpus key
  `3fc202e66ed916686b07f2de2f280a9f`, S2--S3/S7 at 581--1,317 and
  2,459--2,751 seconds** — for a certified antisymmetric matroid, cycles of
  weight at most eight generate the first homology of its transversal base
  graph.  This reduces global carry-holonomy checking to bounded cycles but
  makes the resulting finite potential unsuitable as the escape counter.
- **N. Kamčev, [“Common Linear Patterns Are
  Rare”](https://www.ias.edu/video/common-linear-patterns-are-rare), IAS talk
  (3 Apr. 2023), corpus key `4c4abc8938fa0c999974bc89af1f6c4b`,
  S1--S3/S6 at 600--1,471 and 2,251--2,761 seconds** — an even-girth
  homogeneous finite-field system whose critical equations are all
  nonmatchable is uncommon.  This is an exact row-space no-go for an
  aggregate multiplicity claim only when the Collatz encoding preserves all
  hypotheses.
- **G. Kalai, [“Influences, Traces, Tribes, and Perhaps Also
  Thresholds”](https://www.ias.edu/video/1213/csdm/0204-GilKalai), IAS talk
  (4 Feb. 2013), corpus key `71d73bba75c6b45987972a507d0f0afa`, S6 at
  2,490--2,941 seconds** — Sauer--Shelah exactly relates family size to a
  shattered coordinate trace.  The strict threshold and tight example give a
  finite macro-language audit; trace richness does not select one member
  compatibly across depths.
- **K. Frankston, [“Thresholds Versus Fractional
  Expectation-Thresholds”](https://www.ias.edu/video/csdm/2019/1216-KeithFrankston),
  IAS talk (16 Dec. 2019), corpus key
  `22219e27115af5eef517b28247cba5a6`, S3/S5/S7--S8 at 1,161--1,474 and
  1,889--4,117 seconds** — fractional witness covers, spread measures, and
  their exact LP duality can diagnose concentrated versus dispersed bounded
  failure languages.  Random hitting and residual populations provide no
  nested literal branch.
- **J. Sahasrabudhe, [“The Ramsey Numbers – New Results and New
  Perspectives”](https://www.ias.edu/video/ramsey-numbers-new-results-and-new-perspectives),
  IAS talk (15 Oct. 2024), corpus key
  `03bba7cdc3ba0a71b33fe3d7e5ca8d6e`, S6--S8 at 2,737--4,254 seconds** —
  simultaneous reservoirs and centered neighbourhood-incidence vectors turn
...
- **G. Blanc, [“The Sample Complexity of Smooth Boosting and the Tightness of
  the Hardcore
...
- **J. Listgarten, [“Machine learning-based design (of proteins, small
  ...
  IAS talk (7 July 2020), corpus key `27b7a7f196408ae5d2b8feeee94ad7c8`,
  S3--S8 at 944--3,414 seconds** — distributional optimization equals exact
  pointwise optimization only for a correct oracle and available point masses.
  Learned scores organize proposals but do not certify Collatz paths.
- **P. Chakravarthy, [“Non-squeezing and other rigidity results in LCS
  geometry”](https://www.ias.edu/video/non-squeezing-and-other-rigidity-results-lcs-geometry),
  IAS talk (30 Jan. 2026), corpus key `b79bfb5512996a527a47f7123e67e019`,
  S4--S7 at 1,383--1,627 and 2,668--3,273 seconds** — an integer spectral
  selector yields a monotone capacity and non-squeezing obstruction.  Total
  valuation is analogous but cannot distinguish rational from integral roots.
- **S. Patrikis, [“Compatibility of Canonical Local Systems and F-isocrystals
  on Shimura
  Varieties”](https://www.ias.edu/video/compatibility-canonical-local-systems-and-f-isocrystals-shimura-varieties),
  IAS talk (6 Apr. 2026), corpus key `2e9d2369815a61366064f1ce4aff732f`,
  S3--S7 at 1,128--1,702 and 2,500--3,613 seconds** — compatibility in a
  quotient need not lift through a central cover.  Projected valuation data
  similarly does not determine the ordered affine carry.
- **M. Kerz, [“Proper base change for zero
  cycles”](https://www.ias.edu/video/tav/2015/0217-MoritzKerz), IAS talk
  (17 Feb. 2015), corpus key `6e11bbd99a978ea0dfbe646aea9012f3`, S4--S7 at
  1,165--2,987 seconds** — a base-change isomorphism requires good-position
  moving, surjective lifting, choice independence, and descent of relations.
  Finite Collatz residue lifts meet only the first existence obligation.
- **Z. Lin, [“Polynomially Effective Equidistribution for Some Higher
  Dimensional Unipotent
  Subgroups”](https://www.ias.edu/video/polynomially-effective-equidistribution-some-higher-dimensional-unipotent-subgroups),
  IAS talk (5 May 2026), corpus key `8d921d5a6b10190dee57b396aae24c7d`,
...
- **C. Eur, [“Log-concave Rainbows and Where to Find
  Them”](https://www.ias.edu/video/log-concave-rainbows-and-where-find-them),
  IAS talk (19 May 2025), corpus key `163586c05361260c676440be33ff190e`,
  S1--S7 at 118--3,966 seconds** — Lorentzian rainbow-panel counts give exact
  no-internal-zero and log-concavity tests for genuinely independent colored
  slots.  Ordered carry-coupled Collatz macros require a separately proved
  factorization before any exchange can be inferred.
- **M. Aires, [“Balancing Extensions in Posets of Large
  Width”](https://www.ias.edu/video/balancing-extensions-posets-large-width),
  IAS talk (22 Sept. 2025), corpus key `01cf0536e5763d2bf5e71bfc5be8d175`,
  S2--S7 at 639--3,814 seconds** — balance theorems support exact constant-
  fraction branching over finite dependency schedules.  Linear extensions
  are not literal macro executions, and the one-third/two-thirds and
  Kahn--Saks statements remain conjectures.
- **M. Chudnovsky, [“Induced Subgraphs and
  Pathwidth”](https://www.ias.edu/video/induced-subgraphs-and-pathwidth), IAS
  talk (18 Nov. 2024), corpus key `254a8e42dc061a6137dbef2dabba508a`,
  S5--S8 at 2,715--3,938 seconds** — large pathwidth forces explicit induced
  obstruction families and can reject bounded-memory compatibility graphs.
  A coherent pathwidth-one cylinder tower may still have only the 2-adic root
  `-1`.
- **S. Ghosh, [“Rigidity phenomena in random point sets and
  applications”](https://www.ias.edu/video/nedrm/2013/1211-SubhroshekharGhosh),
  IAS talk (11 Dec. 2013), corpus key `20f9476edaa3b8ea333530a85e7ed2d3`,
  S2--S7 at 527--3,192 seconds** — outside data reconstructs exact counts or
  counts plus sums while leaving conditional tolerance.  Collatz needs the
  order-sensitive affine carry; symmetric valuation statistics identify
  distinct branches, and finite rigidity need not survive a limit.
- **D. Cristofaro-Gardiner, [“Subleading Asymptotics of ECH Capacities and
  Symplectic Packing
  Problems”](https://www.ias.edu/video/subleading-asymptotics-ech-capacities-and-symplectic-packing-problems),
  IAS talk (25 Nov. 2025), corpus key `5ebc61d362c352ac3ce7fbcf36fb9c2c`,
  S1--S5 at 52--4,233 seconds** — a two-term capacity law yields a perimeter
  obstruction to full fillings.  Dyadic density and perimeter cannot
  distinguish a stabilizing ordinary cylinder from the tower converging to
  `-1`; the final tiling obstruction is work in progress.
- **T. de la Rue, [“On a Conjecture of Veech About Möbius
  Orthogonality”](https://www.ias.edu/video/conjecture-veech-about-mobius-orthogonality),
  IAS talk (28 Feb. 2023), corpus key `bc05cf1872ea74e58044f205286e98bd`,
  S1--S8 at 15--3,583 seconds** — maximal characteristic factors control
...
- **D. Sinapova, [“Combinatorics of Singular
  Cardinals”](https://www.ias.edu/video/combinatorics-singular-cardinals), IAS
  talk (6 Apr. 2026), corpus key `046391ca22a18d609fd6375741264028`, S2--S8
  at 521--4,259 seconds** — ZFC and relative-consistency results sharply
  separate finite, countable, and ordinary stationary reflection at successors
  of singulars; some uncountable-cofinality extensions are explicitly
  unchecked.  Passing every fixed finite Collatz audit has the wrong
  quantifier order for one cutoff-independent all-depth certificate.
- **A. Rapinchuk, [“On the notion of genus for division algebras and
  algebraic groups”](https://www.ias.edu/video/puias/2017/1102-AndreiRapinchuk),
  IAS talk (2 Nov. 2017), corpus key `c1b7ac1b1b8bebe1f3ae69e4244457b3`,
  S2--S8 at 846--3,309 seconds** — ramification support plus a finite
  unramified kernel proves finite genus over finitely generated fields, with
  group analogues via good reduction.  Local Collatz fingerprints identify at
  most a finite ambiguity class unless kernel triviality, replay, and an
  ordinary-root bound are separately proved.
- **F. K. Richter, [“Higher Order Sumsets in Sets of Positive
  Density”](https://www.ias.edu/video/higher-order-sumsets-sets-positive-density),
  IAS talk (2 Mar. 2023), corpus key `8824db3f2907e88de617042e795eb866`,
  S1--S7 at 39--3,340 seconds** — correspondence and Host--Kra cubes prove
  infinite `k`-fold sumsets inside every positive-density subset of the
  naturals.  Every finite arithmetic Collatz cylinder already has this
  property, including the all-odd tower whose only coherent root is `-1`.
- **L. C. Lau, [“Solution to the Paulsen problem (via operator
  scaling)”](https://www.ias.edu/video/OCIT/2018/0606-LapChiLau), IAS talk
  (7 June 2018), corpus key `d351230825c66e79deea88d501cb0875`, S1--S8 at
  92--4,283 seconds** — continuous operator scaling, an invariant capacity,
  and smoothed analysis bound movement to an exact balanced frame.  For
  Collatz a useful defect must be integer-valued with an explicit gap;
  continuous balance can converge to a nonintegral rational cycle.
- **P. Engel, [“Matroids and the Integral Hodge Conjecture for Abelian
  Varieties”](https://www.ias.edu/video/matroids-and-integral-hodge-conjecture-abelian-varieties),
  IAS talk (9 Mar. 2026), corpus key `58452424cf273fb4af4cb256fa921ca9`,
  S1--S7 at 30--3,740 seconds** — degeneration and monodromy turn a putative
  algebraic curve into a universal colored-graph system with finite-field
  obstructions.  Such an obstruction can reject a Collatz architecture only
  after an exact ordinary-seed-to-graph implication; local solvability does
  not reverse to an integer realization.
- **O. Gabber, [“Geometry and Arithmetic - Finiteness Theorems for Etale
  Cohomolgy of
  Excellent”](https://www.ias.edu/video/Geometry-and-Arithmetic-Gabber), IAS
  talk (17 Oct. 2005), corpus key `8dc5047e0adbab544f20bc09d787aa98`,
  S1--S8 at 11--3,893 seconds** — alterations, log-regularization, purity, and
  descent prove constructibility and boundedness without general resolution
...
- **M. Amou, T. Matala-aho & K. Väänänen, [“On Siegel--Shidlovskii's theory
  ...
  three.
- **I. P. Rochev, [“Linear independence measures for values of certain
  q-series”](https://arxiv.org/abs/1102.2014) (2011)** — gives a broad local
  Archimedean-or-p-adic independence theorem for arbitrary finite families,
  but assumes the q-parameter expands at one chosen place and at no other.
  The successor parameter `3^102/2^138` expands at both infinity and `2`, so
  the theorem is a precise near-hit rather than an application.  Adapting its
  auxiliary forms to these two places and to the one forced coefficient
  vector is the live literature-driven attack.
