...
- **KL-calibrated recharge/discharge rays.**  Treat the positive class-8
  ...
  target alphabet cannot be invariant; every omitted dyadic-open class meets
  every ternary cylinder by CRT.  The all-symbol Kraft mass is below
  `2^-53`, but this does not exclude an unbounded recursive selector.
...
| Finite ternary-cylinder selector with a fixed writer--decoder alphabet | Universally inadequate, research-side pending QM170.  The exact two-edge map is `Q=q0+9*2^(D'+1)n`, `Q'=q0'+2*3^(g+4)n`; imposing the next symbol gives `n=n0+2^D''*m`, `u=u0+3^(g+2)*m`.  A fixed target symbol occupies one dyadic cylinder of mass `2^-D`; distinct guards are disjoint and the all-symbol Kraft sum is `<2^-53`.  Any finite target list leaves a dyadic-open hole, which CRT places inside every nonempty ternary parameter cylinder.  Schema v2 checks 729 coefficient maps and rejects anchored precisions `k=0..6` by their smallest exact closure failures.  This does not exclude an unbounded recursively selected symbol or certify an invariant. | [`outward_writer_decoder_invariant_cegis_audit.json`](experiments/kontorovich/outward_writer_decoder_invariant_cegis_audit.json), [`kl-calibrated-escape.md`](docs/notes/kl-calibrated-escape.md#32-the-exact-two-edge-parameter-map-and-a-finite-cylinder-obstruction) |
