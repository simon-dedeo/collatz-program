...
| all-place height ledger / ordinary-root promotion gate | Michal Szachniewicz, *Extensions of Globally Valued Fields and Arithmetic Geometry*, key `6576d96faf857ca2824dc88c57a2a6ca`, S2--S7/E8--E9, 654--3,842 s | Attach to every coherent finite seed representative its exact Archimedean size and all relevant prime valuations, not merely its `2`- and `3`-adic precision.  For two representatives congruent modulo `M=2^A 3^B`, certify the elementary product-formula shadow: either their difference is zero or its absolute value is at least `M`.  Search may score small global height, while commit `8c8f249` promotes compatible representatives of uniformly bounded height and growing `M` to one eventually constant ordinary seed. | The talk classifies global height structures by `L^1`-valued valuations/product-formula measures and studies existential closure over algebraic field extensions.  Those results neither construct integer representatives nor make bounded height automatic.  Allowing algebraic extensions changes the target.  Unbounded representative height is consistent with a coherent profinite tower and is not counterexample evidence; omitted prime places can carry compensating mass. |
| exact architecture decomposition / nonlinear-interaction witness | Patricia J. Klein, *Geometric Vertex Decomposition*, key `17434ed77a65bdb05fd1a626a92d0bda`, S1--S8/E4--E7/E10, 49--4,074 s | Encode a bounded selector/invariant parameter family by an exact polynomial ideal and choose the newest feature or carry parameter `y`.  Compute a `y`-compatible Groebner basis.  When the initial `y`-ideal is exactly the certified intersection of its colon/link and deletion components, recurse on both smaller ideals and lift every candidate.  Otherwise return the first nonlinear-in-`y` generator, mixed-height component, or failed equality as a CEGIS witness that this architecture bit does not separate cleanly. | The source theorem requires recursive geometric vertex decomposability and unmixedness to conclude radicality and Cohen--Macaulayness; its example `(xy,yz)` shows that one successful split does not imply unmixedness.  Collatz legality also contains inequalities, congruences, valuation branches, direction, and ordinary-integral points that an algebraic closure can add or erase.  Thus the decomposition can organize a finite architecture search, but cannot certify a macro or seed without exact integer extraction and literal replay. |
| bounded-error growth invariant / semiconjugacy section gate | Rafael Potrie, *Coarse Dynamics and Partially Hyperbolic Diffeomorphisms in 3-Manifolds*, key `d4d546e4a82331f5d506c466c3652618`, S1--S7/E1/E4--E6, 940--3,480 s | Synthesize a rational mixed state coordinate and uniform constants `lambda>1`, `C>=0` such that every selected literal macro satisfies `u(next)>=lambda*u-C`.  Require a start above `C/(lambda-1)` and separately prove bounded counter implies bounded `u`.  If the inequality is discovered on an expanding quotient model, enumerate every fiber and supply a projection-compatible literal section with exact carries. | Potrie's structure theorem deliberately permits a continuous collapse map that is not a homeomorphism and can identify distinct flow orbits.  Model expansion therefore supplies no Collatz execution without a section.  The source uses partial hyperbolicity, foliations, pseudo-Anosov dynamics, and compact-manifold topology; only the exact inequality and fiber audit transfer.  Uniform all-precision constants, a bounded ordinary root, and counter coercivity remain unproved. |
| recursive first-return tower / reject measure joining as decoder | Jonathan Chaika, *There Is a Mildly Mixing Rank 1 Transformation with an Interesting Joining*, key `c79b0de06ac4f2fae91a428c79846fda`, S1/S3--S8/E3--E6, 50--3,810 s | A proposed variable-base tower must be a literal recursive first-return grammar: every cut/spacer block replays positively, returns only at its declared boundary, projects to the previous word, and records the mixed-base root carry.  For any two-coordinate coupling, enumerate finite fibers and require compatible singleton fibers before treating it as a decoder. | Rank-one towers approximate measurable sets modulo null sets.  Chaika's explicit construction is only sketched as a first return of a variable-base odometer, and its new joining is a weak-star limit whose conditional measure has two atoms per fiber.  It is therefore not a pointwise graph.  Tower height, measure coverage, and a structured joining prove neither counter growth along one address nor bounded ordinary-root stabilization. |
| canonical local-certificate aggregation / symmetry-extension gate | László Babai, *Graph Isomorphism in Quasipolynomial Time II*, key `cb7d0cee424a5e69b74805cab8bcab82`, S1--S7/E1, 53--4,090 s | On the complete directed, edge- and carry-labeled finite macro graph, admit a local symmetry only after it extends to an actual global automorphism.  Aggregate fullness or nonfullness certificates over every ordered test tuple and verify functoriality under every declared relabeling.  If several maximal block systems survive, retain the entire canonical set and branch over it; return the first failed global extension or naturality square. | Babai's algorithm assumes an ambient permutation group, exact string automorphisms, local-certificate theorems, and downstream design/Johnson reductions.  Collatz dynamics is noninvertible and is not itself this group.  A canonical partition creates no legal edge, and per-level isomorphism functoriality does not imply compatibility across precision, counter advance, or an ordinary root. |
| finite-radius approximation/separation ledger / wraparound warning | Michael Chapman, *Subgroup Tests and the Aldous--Lyons Conjecture*, key `bf4f08cdf938265d74ded1ab8f1bd905`, S1--S7/E1--E7, 54--4,410 s | For any bounded-degree finite-label macro architecture, compute exact rational rooted-ball frequency vectors.  Emit either an explicit finite model within a stated `(radius,epsilon)` bound or a rational cylinder functional with a universally proved gap over all finite legal models.  Preserve directed macro/carry labels and check projective consistency and full mass transport. | A finite cycle can label almost every local edge as counter `+1` while hiding one wraparound defect of vanishing density; every fixed-radius limit then looks outward although no finite counter is unbounded.  Uniform rerooting also forgets the unique seed.  The IRS/Schreier bridge requires an invertible group action, and the talk's undecidability route has additional hypotheses.  Local weak approximation or separation therefore proves neither literal escape nor ordinary-root realization. |
| exact recurrence-spectrum certificate / reject floating transfer modes | Olga Holtz, *Zeros of Polynomials via Matrix Theory and Continued Fractions*, key `224b5b6eb3422637b5925da6a65947a8`, S1--S8/E1--E4, 58--2,986 s | When a literal parametric policy has first been proved to induce an exact rational linear recurrence for a mixed state coordinate, use a signed polynomial remainder/Sturm sequence to isolate real roots and use exact Hurwitz principal-minor signs, after a Cayley transform when needed, to certify half-plane or unit-disk location.  Replay the initial condition to prove that a certified expanding root has a nonzero coefficient and that bounded counter would bound the coordinate. | The criteria say nothing until the recurrence itself is semantic.  On a nondeterministic transfer matrix an expanding eigenvalue can count many finite paths rather than select one nested path, and on a nonnormal matrix numerical modes are unreliable.  Cancellations, address-only growth, affine forcing, carries, and roots on the boundary require separate exact treatment.  The talk's `m>2` sector criterion is only one-way, as its Q&A explicitly notes. |
| rejected direct capacity transfer / localized-potential axiom gate | Sam Lisi, *Lagrangian Hofer--Zehnder Capacities and Energy--Capacity Inequalities*, key `54865c0e0393026e80420023ec3bd540`, S1--S7/E2/E5--E7, 86--4,118 s | Before calling any finite return/resource statistic a capacity, define it on exact macro-state/section pairs and prove monotonicity under literal pair embeddings, normalization, and the claimed inequality to a replayable displacement cost.  Require the high-potential locus to be one controlled root/counter neighborhood rather than a plateau along an inert address or carry factor; otherwise return that plateau as a coercivity failure. | Lisi's inequality uses monotone Lagrangians, Floer/pearl homology, action filtrations, and a ball-shaped maximum set.  The talk gives a displaceable nonmonotone example of infinite capacity and an unhatted example where a large Hamiltonian misses the displacement direction.  Collatz has no supplied symplectic pair, Hamiltonian displacement, or Floer differential.  The examples transfer as hypothesis audits only; no capacity bound or escape conclusion does. |
| collective linear routing obstruction / noncommutative-rank verifier | K.V. Subrahmanyam, *An Algebraic Algorithm for Non-Commutative Rank over Any Field*, key `aaa7ccd726bcf6860820e05a07293575`, S1--S7/E1--E3, 98--4,082 s | Encode each replayed directed macro template as a source-to-target matrix.  Demand either an exact shrunk-subspace pair `U,U'` with every template sending `U` into `U'` and `dim(U)>dim(U')`, or an exactly invertible matrix blow-up proving full noncommutative rank.  A Wong sequence constructs/verifies the former; exact rank or determinant verifies the latter. | Coordinate shrunk subspaces recover Hall blockers, while general subspaces can expose linear carry/residue bottlenecks.  Full noncommutative rank is not a route: the blow-up uses noncommuting controller matrices, field addition, and superposition of macro templates.  A separate integral/literal routing theorem is required, as are compatible cross-layer witnesses and an ordinary root.  Field-size, extension, and divisibility hypotheses must be stated. |
| shallow-selector architecture rejection / sparse restriction hybrid | Avishay Tal, *Oracle Separation of Quantum Polynomial Time and the Polynomial Hierarchy*, key `acf6ab355b16ef6187db23054912baa5`, S1--S8/E5/E7, 32--4,159 s | Only for a selector bit compiled as a stated bounded-size/depth AND/OR/NOT circuit, define exact complete-state null and correlated-lift distributions, certify restriction closure and sparse low-degree Fourier approximants, and sum exact moment/hybrid bounds.  If the required distinguishing margin exceeds the certified total, reject that shallow feature architecture and use the first large restriction/monomial as a refinement witness. | Tal's theorem is an oracle separation using a Gaussian/Hadamard forrelation distribution and a quantum superposition query; none transfers directly.  Parity, threshold, modular, recursive, or large tabular selectors leave the stated AC0 class.  A finite distinguisher lower bound rejects only that compiled circuit architecture and provides no legal macro, coherent counter lift, or ordinary root. |
| exact stationary-kernel/lumpability audit / reject entropy as decoder | Tianyi Zheng, *Furstenberg Entropy Spectrum of Stationary Actions*, key `6a68c2efc7ece967b34da84b87a8e520`, S1--S8/E1--E3, 15--3,620 s | For a randomized finite macro policy, verify a rational stationary kernel, stationary law, cross-precision factor equations, and exact lumpability of any proposed feature partition.  A literal successor is decoded only on singleton conditional edge support; otherwise require a separate legal selector commuting with every projection and return the first nonsingleton fiber or failed naturality square. | Furstenberg entropy averages over a group-driven random walk and a stationary measure.  Entropy intervals, factor monotonicity, Poisson boundaries, and Doob transforms describe path laws, not one deterministic feedback-selected Collatz execution.  Compatible stationary measures can coexist with incompatible or unbounded root representatives; counter growth, exact carries, and ordinary-root stabilization remain independent. |
| closed strong-mixing tower lane / finite-factor obstruction | Rigoberto Zelada, *Polynomial Ergodic Theorems for Strongly Mixing Commuting Transformations*, key `5b8bffb894ee39980c594c09c9a27342`, S1--S8/E1--E5, 14--3,385 s | Before importing polynomial mixing, verify total bijections, measure preservation, commutation, and closure of every inverse/difference word generated by PET.  More decisively, reject any strongly mixing model that descends through a nontrivial finite residue factor: the induced finite permutation has an identity power, giving a set whose correlation is `mu(A)` rather than `mu(A)^2`. | Literal Collatz macros are partial, generally noninvertible and noncommuting.  The coherent finite factors needed for seed bonding contradict strong mixing itself, so longer correlation searches cannot repair the hypothesis.  Uniform-density and ultrafilter limits also ignore sparse exceptional times and do not select a root.  PET algebra may list exact commutator failures, but its convergence theorem cannot drive this tower. |
...
- Michal Szachniewicz, *Extensions of Globally Valued Fields and Arithmetic
  Geometry*, IAS, 9 Oct. 2025, key `6576d96faf857ca2824dc88c57a2a6ca`,
  https://www.ias.edu/video/extensions-globally-valued-fields-and-arithmetic-geometry.
- Patricia J. Klein, *Geometric Vertex Decomposition*, IAS, 4 Dec. 2024, key
  `17434ed77a65bdb05fd1a626a92d0bda`,
  https://www.ias.edu/video/geometric-vertex-decomposition.
- Rafael Potrie, *Coarse Dynamics and Partially Hyperbolic Diffeomorphisms in
  3-Manifolds*, IAS, 3 Feb. 2020, key `d4d546e4a82331f5d506c466c3652618`,
  https://www.ias.edu/video/members/2020/0203-RafaelPotrie.
- Jonathan Chaika, *There Is a Mildly Mixing Rank 1 Transformation with an
  Interesting Joining*, IAS, 24 Mar. 2026, key
  `c79b0de06ac4f2fae91a428c79846fda`,
  https://www.ias.edu/video/there-mildly-mixing-rank-1-transformation-interesting-joining.
- László Babai, *Graph Isomorphism in Quasipolynomial Time II*, IAS, 1 Mar.
  2016, key `cb7d0cee424a5e69b74805cab8bcab82`,
  https://www.ias.edu/video/csdm/2016/0301-Babai.
- Michael Chapman, *Subgroup Tests and the Aldous--Lyons Conjecture*, IAS,
  7 Oct. 2024, key `bf4f08cdf938265d74ded1ab8f1bd905`,
  https://www.ias.edu/video/subgroup-tests-and-aldous-lyons-conjecture.
- Olga Holtz, *Zeros of Polynomials via Matrix Theory and Continued
  Fractions*, IAS, 24 Feb. 2014, key `224b5b6eb3422637b5925da6a65947a8`,
  https://www.ias.edu/video/members/2014/0224-OlgaHoltz.
- Sam Lisi, *Lagrangian Hofer--Zehnder Capacities and Energy--Capacity
  Inequalities*, IAS, 5 May 2026, key `54865c0e0393026e80420023ec3bd540`,
  https://www.ias.edu/video/lagrangian-hofer-zehnder-capacities-and-energy-capacity-inequalities.
- K.V. Subrahmanyam, *An Algebraic Algorithm for Non-Commutative Rank over Any
  Field*, IAS, 6 June 2018, key `aaa7ccd726bcf6860820e05a07293575`,
  https://www.ias.edu/video/OCIT/2018/0606-K.V.Subrahmanyam.
- Avishay Tal, *Oracle Separation of Quantum Polynomial Time and the
  Polynomial Hierarchy*, IAS, 1 Oct. 2018, key
  `acf6ab355b16ef6187db23054912baa5`,
  https://www.ias.edu/video/csdm/2018/1001-AvishayTal.
- Tianyi Zheng, *Furstenberg Entropy Spectrum of Stationary Actions*, IAS,
  12 Nov. 2024, key `6a68c2efc7ece967b34da84b87a8e520`,
  https://www.ias.edu/video/furstenberg-entropy-spectrum-stationary-actions.
- Rigoberto Zelada, *Polynomial Ergodic Theorems for Strongly Mixing Commuting
  Transformations*, IAS, 4 Apr. 2023, key
  `5b8bffb894ee39980c594c09c9a27342`,
  https://www.ias.edu/video/polynomial-ergodic-theorems-strongly-mixing-commuting-transformations.
