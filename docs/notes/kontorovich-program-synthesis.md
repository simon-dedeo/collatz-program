...
Lean commit `768f4d0` kernel-checks (8.1), the signed `(-1,[1])` controller,
the three outward finite macrosteps and their level-10 failure, and the literal
Collatz negation from any supplied infinite `MersenneShadowOrbit`.  Thus the
remaining special-case problem is exactly the all-level recurrence, not its
soundness bridge.

The first feedback grammar is constant `e_M=e`.  Exhausting start levels
through 20, `e<=32`, and depth through 40 gives `51,200` compiled paths.  The
only two-extension stabilization is
...
The same seed is canonical for the depth-two, depth-three, and depth-four
