...
`docs/notes/boundary-jordan-mahler.md`.  A general 2-adic lifting theorem is
no longer required: the degree-one Padé remainder `(1-CZ)H-1` cancels through
`Z^16`, and the resulting exact precision/height identity gives an elementary
irrationality proof for the eight prescribed values.  The functional identity
is kernel-checked and a 32,776-row exact audit verifies the exponent algebra;
the final all-level rational-height contradiction awaits Lean.  Keep the
worker only as a conditional fallback.  Do not spend PSC or local cores on a
generic transducer search while this much narrower proof seam is being
formalized.
