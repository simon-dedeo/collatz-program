...
#pragma omp parallel for schedule(static)
    ...
    FILE*out=fopen(argv[3],"w");if(!out)fail("cannot open output");fprintf(out,"# exact exhaustive zero-carry canonical-cylinder shard prefix_depth=%u total_schedules=%llu shard_index=%llu shard_count=%llu shard_schedules=%llu positive_representatives=%llu replay_cap=%u threads=%d seconds=%.6f overflowed_replays=%llu arithmetic_bits=256\n",depth,(unsigned long long)count,(unsigned long long)shard,(unsigned long long)shards,(unsigned long long)shard_schedules,z.positive,cap,nt,omp_get_wtime()-start,z.overflow);for(unsigned d=0;d<=cap;++d)if(z.hist[d])fprintf(out,"HIST\ttotal_survival=%u\tbeyond_prefix=%u\tcount=%llu\n",d,d>=depth?d-depth:0,z.hist[d]);fprintf(out,"CHAMPION\ttotal_survival=%u\tbeyond_prefix=%u\tseed=",z.max_extra,z.max_extra>=depth?z.max_extra-depth:0);print256(out,z.seed);fprintf(out,"\tprefix_endpoint=");print256(out,z.endpoint);fprintf(out,"\tcounterexample=null\n");fclose(out);free(s);return 0;}
